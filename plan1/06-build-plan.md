...
### ISP-033A · Whish Pay Lebanon gateway (planned)
Add a standalone PHP package under `packages/whish-pay-php` (PSR-4, no Node runtime and no browser-side merchant secret) based on the referenced community client’s server-side contract. Integrate it through the `PaymentGateway` boundary with sandbox/production configuration, channel and secret handling, timeout/error mapping, exact currency-unit conversion, and a persisted Whish external ID/payment-attempt record created before the provider request. The collector payment flow must create a Whish collect URL, render a QR code that opens it, and expose a safe status/polling view without treating a redirect as proof of payment. Success/failure callbacks parse only the provider identifiers, re-query Whish for authoritative status, verify amount and currency against the stored attempt, and post a successful payment through `RecordPayment` with the gateway/external ID as an idempotency key. USD/LBP (and any provider-supported currency) must retain the saved rate, source, effective date and rounding policy already used by the ledger.
**AC:** a collector can generate a QR for a stored Whish attempt; the PHP client never logs or serializes the merchant secret; callback query parameters alone cannot post a payment; a provider-confirmed success posts exactly one allocated `gateway` payment after amount/currency validation; repeated callbacks and status polls are safe; sandbox and production endpoints are selected explicitly; HTTP fakes cover create/status/failure/timeout paths. Verify endpoint paths, authentication headers, payload units and callback fields against current official Whish merchant documentation before enabling production.
