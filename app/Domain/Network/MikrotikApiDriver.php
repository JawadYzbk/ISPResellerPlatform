...
final class MikrotikApiDriver implements NetworkDriver
{
    ...
    public function execute(Service $service, NetworkCommand $command): DriverResult
    {
        ...
        if (in_array($command->action, ['activate', 'suspend', 'pause', 'throttle', 'change_plan'], true) && $reference === null) {
        ...
        if (in_array($command->action, ['suspend', 'pause', 'throttle', 'change_plan'], true) && $reference === null) {
            return DriverResult::failure('router_subscriber_not_found', ['action' => $command->action, 'router_id' => $router->id]);
        }

        if (! in_array($command->action, ['activate', 'suspend', 'pause', 'throttle', 'change_plan'], true)) {
