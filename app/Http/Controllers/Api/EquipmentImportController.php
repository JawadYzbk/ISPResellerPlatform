...
{
    public function store(Request $request, ImportEquipmentCsv $import, NormalizeTabularImport $normalize, ImportBatchApiResource $resource): JsonResponse
    {
        ...
        $filename = (string) ($validated['filename'] ?? $validated['file']?->getClientOriginalName() ?? 'equipment.csv');
    ...
    {
        abort_unless($request->user()?->can('inventory.receive'), 403);
        $batch = ImportBatch::query()->where('public_id', $import)->where('type', 'equipment')->firstOrFail();
        $deleted = $rollback->handle($batch);

        return response()->json(['id' => $batch->public_id, 'status' => $batch->refresh()->status, 'deleted_equipment' => $deleted]);
    }
}
