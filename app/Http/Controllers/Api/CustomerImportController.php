...
{
    public function store(Request $request, ImportCustomersCsv $import, NormalizeTabularImport $normalize, ImportBatchApiResource $resource): JsonResponse
    {
        ...
        $filename = (string) ($validated['filename'] ?? $validated['file']?->getClientOriginalName() ?? 'customers.csv');
    ...
    public function rollback(Request $request, string $import, RollbackImport $rollback): JsonResponse
    {
        ...
        return response()->json(['id' => $batch->public_id, 'status' => $batch->refresh()->status, 'deleted_customers' => $deleted]);
    }
}
