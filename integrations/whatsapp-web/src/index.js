...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (request.method === 'GET' && request.url === '/qr') {
      ...
      return json(response, status.status === 'qr' ? 200 : 404, status.status === 'qr' ? { qr: status.qr } : { error: 'qr_unavailable' });
    }
    if (request.method === 'POST' && request.url === '/messages') {
      const payload = await readJson(request);
      const result = await bridge.send({ idempotencyKey: payload.idempotency_key, to: payload.to, body: payload.body });
