...
Use **Operations -> Work orders** to open the detail history, review the checklist and uploaded site evidence, reschedule an active job in tenant-local time, and complete assigned installations or repairs. Evidence files are private to the tenant and can be downloaded from the work-order detail page. Rescheduling requires the work-order capability and recent authentication and records a history event. Installation completion activates the linked service and queues network activation. Use **Operations -> Inventory** to trace serialized equipment and confirm whether a unit is still available or assigned to a service. Staff with the inventory-assignment capability can select an eligible service for an available unit; assignment requires recent authentication and records an auditable movement.
