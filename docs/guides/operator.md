...
Use **Collector routes** to build each collector's ordered daily stop list inside their assigned territory. Use **Collector tasks** for work that needs explicit ownership or discussion: choose the collector, optionally attach a customer, set priority and due time, and keep instructions in the task thread. Task files remain private to operations managers and the assigned collector. At checkout, ask the collector to add a concise handover note; **Collector check-ins** retains the resulting route, payment, task, cash-shift and duration summary for supervision.

Use **Collector custody** for physical cash responsibility. Cash gateway records are excluded; only posted cash collections, opening float, advances, approved expenses, handovers and adjustments affect the displayed position. Collector expense and handover requests remain pending until a manager approves them. Confirm the count and reference before approval: posted entries are append-only, and the system rejects any debit that would make the collector's currency balance negative.
...
Use **Operations -> Routers** to register devices with a tenant POP, open a router detail page, filter them by online/offline state, and run a bounded health check. Router API and optional RADIUS/CoA source secrets are encrypted at rest and are not rendered in the operations list or detail response. When a RADIUS secret is configured, the router lifecycle synchronizes a stock FreeRADIUS `nas` row for the private FreeRADIUS SQL connection; the runtime row is redacted from application responses and must be protected with database/network least privilege. The detail page shows the last successful observation, consecutive failures, assigned service count and TLS verification state. Updating connection settings requires recent authentication; blank secret fields preserve the encrypted values. Use **Operations -> Network queue** to follow the resulting command and its retry history.

FreeRADIUS accounting rows are imported from `radacct` every five minutes, mapped to the tenant service by username and NAS, and mirrored into the live sessions view. The daily usage rollup runs before quota enforcement; if a tenant uses router polling instead of FreeRADIUS accounting, the existing `sessions_current` counters remain the fallback source. Verify the scheduler and queue-worker health before treating a live-session or quota screen as current.
