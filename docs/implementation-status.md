...
| ISP-020/021 | Customer seed, index/show screen, staff create/edit flows, working list filters/columns, real activity timeline, persisted provisioning details and tenant-isolated search entry point | vertical slice delivered |
...
| ISP-030/031/032/033 | Historical FX, append-only double-entry ledger, invoice lifecycle, idempotent payments and reversals, staff invoice queue/draft issuance and staff payment collection with invoice-balance validation | delivered |
...
| ISP-060/061/062 | Ticket lifecycle, tenant-timezone business-hours SLA clock, scheduled resolved-ticket auto-close, tenant-safe staff ticket queue/detail/reply/status workflow, customer portal ticket messages, work-order completion and serialized inventory assignment | delivered for current ticket/work-order scope |
...
| ISP-073 | Technician-assigned work-order list/detail endpoints, tenant-safe staff work-order completion surface, idempotent completion, separate image upload, service diagnostics and serialized inventory operations view | delivered for current technician scope |
...
| ISP-040/063 | POP/router inventory, encrypted connection tests, repeated-failure incidents, bounded router health observations and tenant-safe staff router operations page | foundation delivered |
...
| ISP-P1-01 | Supplier credential inventory web surface, import, assignment and audited reveal | foundation delivered; inventory surface delivered, reveal/assignment rollout remains controlled |
