...
| ISP-040/063 | POP/router inventory and capability-gated onboarding, tenant-safe POP and upstream-link inventory/detail, tenant-safe router detail/update, encrypted connection tests, repeated-failure incidents, bounded router health observations and tenant-safe staff router operations page | delivered for current registry scope; IP pools/addresses and POP write workflows remain |
