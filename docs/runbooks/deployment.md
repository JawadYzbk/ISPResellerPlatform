...
Set `FX_RATE_MAX_AGE_HOURS` to the approved treasury freshness window for the market (72 hours by default). The owner-facing **Settings → Pilot readiness** checklist warns when the effective collection rate is older than that window; it does not overwrite or silently replace a manual rate.
