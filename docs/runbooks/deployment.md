...
The WhatsApp Web.js bridge is deliberately behind the `whatsapp` Compose profile and the application defaults to the Cloud API path. To opt in, set `WHATSAPP_PROVIDER=web`, `WHATSAPP_WEB_ENABLED=true`, `WHATSAPP_WEB_TOKEN`, and `WHATSAPP_WEBHOOK_SECRET`, then start it with `--profile whatsapp`. Pair the account through the private bridge `/qr` endpoint, verify `/status` reports `ready`, then sign in as the tenant owner, open **Settings → WhatsApp setup**, enter a dedicated operator phone, and send one controlled test message. Confirm the queued message and delivery callback before enabling customer notifications. Keep the session volume private and backed up; the bridge uses an unofficial WhatsApp Web client and may be disconnected or blocked by WhatsApp.

The application also applies delivery-safety controls to WhatsApp messages. Identical recipient/body pairs are suppressed for the duplicate window (120 seconds by default), each account is paced at 8 seconds plus up to 4 seconds of jitter, and account volume is capped at 60 messages per hour and 500 per day. Provider failures trigger an exponential account cooldown from 30 seconds up to 15 minutes, while queued work is deferred instead of being sent through a second channel. The bridge serializes outbound sends and replays a durable idempotency result after a timeout or worker retry. These controls reduce accidental bursts and duplicate sends; they cannot guarantee that WhatsApp will not restrict an account. Use the Web.js path only for consented, expected operational notifications and follow WhatsApp's current policies.
...
`platform:preflight --production` must pass before traffic is admitted. It checks the application key, database connectivity, migration state, production environment, debug mode, public HTTPS URL, secure session cookies, web two-factor enforcement and privileged-staff enrollment, asynchronous queue, persistent cache, tenant capability assignments, Sentry privacy configuration, encrypted off-site backups, private object storage, monitoring alert routing, Reverb credentials when realtime is enabled, and the Web.js bridge configuration when that provider is selected. It does not replace the scheduler, queue-worker, backup-restore, or external alert checks below. Local development may keep `SECURITY_ENFORCE_TWO_FACTOR=false`; production must set it to `true` and verify every owner, billing, operations, network, and reseller administrator has completed enrollment before traffic is admitted.

Run `php artisan platform:provider-check --json` after the provider environment is loaded. It performs read-only connectivity probes for enabled Frankfurter, Stripe, Whish Pay, and WhatsApp Web.js integrations. Resolve every `failed` or `not_configured` result before enabling the corresponding customer-facing flow; disabled providers are reported without being contacted.

Reload every long-lived application process after the release build and cache commands complete. Inertia deferred props are generated by the application process, so the app/PHP-FPM, queue worker, scheduler and Reverb processes must all run the same release before traffic is admitted; otherwise a new frontend bundle can receive a stale deferred payload contract. After the reload, open the dashboard and verify its deferred finance and attention sections render without browser page errors.
...
On 2026-08-11, release commit `583531a` was rebuilt in an isolated `isp-manager-acceptance` project and completed a fresh PostgreSQL 17 migration and demo seed, production config/route/event caching, all 21 `platform:preflight --production` checks, Nginx `/login` and `/docs/api` probes, `/up`, `/api/v1/health`, and the WhatsApp bridge health probe (`status: qr`). The disposable smoke used the internal Compose Reverb service so queued realtime events and worker health were exercised without relying on external DNS. Synthetic containers, volumes, network and environment file were removed after the run. This verifies current image and topology wiring; it does not claim live provider credentials, WhatsApp pairing, or RouterOS acceptance.
