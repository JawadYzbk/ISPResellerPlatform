...
7. Run `php artisan platform:provider-check --json` after loading provider credentials. Resolve every enabled provider result marked `failed` or `not_configured`; this command is read-only and does not create a payment or send a message.
8. Start the pilot only after the owner signs off the customer import, router scope, payment/notification behavior, rollback plan, and two-week monitoring owner. Record the tenant public ID, not internal database IDs, in the handover document.
