...
- The private WhatsApp Web.js bridge exposes `GET /health` inside the Compose network. Alongside readiness and account count, it reports process uptime, active/ready/QR account counters, RSS/heap/external memory in MB, and a `memory` warning when RSS reaches `WHATSAPP_WEB_MEMORY_WARNING_MB` (default `2048`). It intentionally excludes phone numbers, QR payloads, session paths and credentials.
